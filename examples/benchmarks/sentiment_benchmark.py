...
def report(true, pred, modelname):
    data_b = []
    data_a = []
    headers_b = ['class', 'precission', 'recall', 'f1']
    headers_a = ['model', 'accuracy', 'avg-f1', 'weighted-f1']
    ...
    data_a.append([modelname, acc, avg, wei])

    print(tabulate.tabulate(data_a, headers=headers_a, tablefmt='github', colalign=aligns))
    print()
    print(tabulate.tabulate(data_b, headers=headers_b, tablefmt='github', colalign=aligns), '\n')
...
def afinn_benchmark(dataset):
    if dataset == 'euparlsent':
        print(
            'Eutoparl sentiment evaluation set annotated by Finn Årup Nielsen \n')
